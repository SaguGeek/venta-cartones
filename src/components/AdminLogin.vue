  ...
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-900 to-purple-300">
    <div class="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md transform transition-all duration-500 opacity-0 animate-fade-in">
      <div class="text-center mb-6">
        ...
        <h2 class="text-3xl font-bold text-gray-800 mb-2">Login Administrador</h2>
        <p class="text-gray-600">Inicia sesión como administrador</p>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      error: '',
      loading: false,
      adminEmail: 'master1@example.com' // Correo fijo para administrador
    };
  },
  methods: {
    async login() {
      ...
      try {
        if (this.email !== this.adminEmail) {
          throw new Error('Solo el administrador master1@example.com puede iniciar sesión aquí.');
        }
